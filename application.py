...
from alibi_detect.cd import KSDrift  # Kolomogorov-smirov test
from src.feature_store import RedisFeatureStore # This will use to get reference data
...
def predict():
    try:
        ...
        drift = ksd.predict(features_scaled) ## ksd train with reference data. In this line, ksd will predict the drift using new data
        print("Drift Response : ", drift)

        drift_response = drift.get('data',{})
        is_drift = drift_response.get('is_drift' , None)

        # if the "is_drift" = 1, there is a data drift
