...
class DataProcessing:
    ...
    def handle_imbalance_data(self):
        ...
        """
            Handle the imbalancdness of the training Dataset
        """

        try:
            X = self.data[['Pclass', 'Sex', 'Age', 'Fare', 'Embarked', 'Familysize', 
    ...
    def store_feature_in_redis(self):
        ...
        """
            Store features in the REDIS (Preprocessed data)
        """

        try:
            batch_data = {}
            for idx,row in self.data.iterrows():
                entity_id = row["PassengerId"]  ## This entity column should be unique for each row in the dataset
                features = {
                    "Age" : row['Age'],
                    "Fare" : row["Fare"],
                    "Pclass" : row["Pclass"],
            ...
            logger.info("Data has been feeded into Feature Store as batches..")
        except Exception as e:
...
if __name__=="__main__":
    ...
    ### 332 is the passenger ID of the 1 st data row
